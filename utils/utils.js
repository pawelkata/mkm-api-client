...
const MKM_API_URL = 'api.cardmarket.com'
const MKM_SANDBOX_URL = 'sandbox.cardmarket.com'
...
utils.__mkmrequest = function (keys, method, path, data, headers, tokens) {
	...
	var params = {
		https: true,
		method: method,
		host: utils.debug ? MKM_SANDBOX_URL : MKM_API_URL,
		path: path,
		headers: headers
	};
	if (data) {
...
function getUri(path) {
	return 'https://' + (utils.debug ? MKM_SANDBOX_URL : MKM_API_URL) + path;
}
